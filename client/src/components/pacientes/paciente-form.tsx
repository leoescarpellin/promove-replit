...
export function PacienteForm({ paciente, onClose }: PacienteFormProps) {
  const { toast } = useToast();
  const [tiposAtendimentoPaciente, setTiposAtendimentoPaciente] = useState<any[]>([]);
  ...
  React.useEffect(() => {
    if (paciente && !isLoadingTiposAtendimento) {
      ...
      setTiposAtendimentoPaciente(tiposMapeados);
    }
  }, [paciente, criancaTiposAtendimento, isLoadingTiposAtendimento, tiposAtendimento]);
  ...
  const adicionarTipoAtendimento = (data: TipoAtendimentoClienteValues) => {
    ...
    const jaExiste = tiposAtendimentoPaciente.some(
  ...
  const removerTipoAtendimento = (index: number) => {
    const novosTipos = [...tiposAtendimentoPaciente];
    novosTipos.splice(index, 1);
    setTiposAtendimentoPaciente(novosTipos);
  };
  ...
  const onSubmit = async (data: ClienteFormValues) => {
    try {
      ...
      if (paciente) {
        ...
        await apiRequest('PUT', `/api/criancas/${paciente.id}`, data);
        criancaId = paciente.id;
      ...
      if (paciente) {
        ...
        const tiposAtuais = criancaTiposAtendimento
          .filter((rel: any) => rel.criancaId === paciente.id)
          .map((rel: any) => rel.id);
        
        const tiposIdsFormulario = tiposAtendimentoPaciente
          .filter(t => t.id !== null)
          .map(t => t.id);
      ...
      for (const tipo of tiposAtendimentoPaciente) {
        if (!tipo.id) {
          // Nova relação
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div>
          ...
          {tiposAtendimentoPaciente.length === 0 ? (
            <Card>
            ...
            <div className="space-y-3">
              {tiposAtendimentoPaciente.map((tipo, index) => (
                <Card key={index}>
